  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className="ml-64 flex-1 px-8 py-8">
        <PageHeader title="Verifikasi Pengajuan" breadcrumb={['Dashboard','Detail']} />
          ...
          <div className="card p-6">
            ...
            {data.catatan && (
              <div className="mt-4 p-3 bg-amber-50 rounded-lg">
                <dt className="text-xs text-amber-600 font-medium mb-0.5">Catatan Admin</dt>
                <dd className="text-sm text-gray-700">{data.catatan}</dd>
              </div>
            )}
          </div>
          <div className="card p-6">
            <p className="section-label">Catatan Verifikasi</p>
            <textarea rows={3} value={catatan} onChange={e => setCatatan(e.target.value)}
              placeholder="Tambahkan catatan verifikasi (opsional)..." className="input-field resize-none" />
          </div>
          <div className="flex gap-3">
            <button onClick={() => updateStatus('DIVERIFIKASI')} disabled={loading} className="btn-primary px-6 py-2.5">
              Verifikasi &amp; Teruskan
            </button>
            <button onClick={() => updateStatus('DIPERIKSA')} disabled={loading} className="btn-outline px-6 py-2.5">
              Kembalikan ke Admin
            </button>
          </div>
        </div>
